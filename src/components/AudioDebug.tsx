...
export function AudioDebug(props: Props) {
  ...
  const [signalLevels, setSignalLevels] = createSignal<SignalLevels | null>(
    null
  )
  ...
  const updateDebugInfo = () => {
    if (isOpen()) {
      ...
      setSignalLevels(props.engine.getSignalLevels())
    }
    ...
    <div class="fixed bottom-4 right-4 z-50">
      ...
      <Show when={isOpen()}>
        <div class="absolute bottom-12 right-0 w-96 max-h-[80vh] overflow-auto bg-gray-900 border border-gray-700 rounded-lg p-4 text-xs font-mono">
          <h3 class="text-sm font-bold mb-3 text-gray-200">Audio Graph Debug</h3>

          <Show when={signalLevels()}>
            {(levels) => (
              <Section title="Signal Levels">
                <div class="flex gap-4">
                  <div class="flex-1">
                    <div class="text-gray-400 mb-1">Peak</div>
                    <div class="h-2 bg-gray-800 rounded overflow-hidden">
                      <div
                        class="h-full bg-green-500 transition-all duration-75"
                        style={{
                          width: `${Math.min(100, levels().peak * 100)}%`,
                        }}
                      />
                    </div>
                    <div class="text-gray-500 mt-1">
                      {formatDb(levels().peakDb)} dB
                    </div>
                  </div>
                  <div class="flex-1">
                    <div class="text-gray-400 mb-1">RMS</div>
                    <div class="h-2 bg-gray-800 rounded overflow-hidden">
                      <div
                        class="h-full bg-blue-500 transition-all duration-75"
                        style={{
                          width: `${Math.min(100, levels().rms * 100)}%`,
                        }}
                      />
                    </div>
                    <div class="text-gray-500 mt-1">
                      {formatDb(levels().rmsDb)} dB
                    </div>
                  </div>
                </div>
              </Section>
            ...
            {(info) => (
              <>
                <Section title="Audio Context">
                  ...
                  <Row label="Time" value={`${info().currentTime.toFixed(2)}s`} />
                  <Row label="Master Gain" value={info().masterGainValue.toFixed(2)} />
                </Section>
                ...
                <Section title="Source">
                  ...
                  <Row
                    label="Oscillator"
                    value={info().source.oscillatorActive ? 'running' : 'stopped'}
                    valueClass={info().source.oscillatorActive ? 'text-green-400' : 'text-gray-500'}
                  />
                  <Row
                    label="Microphone"
                    value={info().source.micActive ? 'active' : 'inactive'}
                    valueClass={info().source.micActive ? 'text-green-400' : 'text-gray-500'}
                  />
                </Section>

                <Section title="Analyser">
                  <Row label="FFT Size" value={info().analyser.fftSize} />
                  <Row label="Freq Bins" value={info().analyser.frequencyBinCount} />
                </Section>
                      ...
                      {info().stageInstances.map((stage, i) => (
                        <div
                          class={`flex items-center justify-between p-1 rounded ${
                            stage.bypassed ? 'bg-gray-800 opacity-50' : 'bg-gray-800'
                          }`}
                        >
                          <span>
                            {i + 1}. {stage.kind}
...
function Row(props: { label: string; value: string | number; valueClass?: string }) {
  return (
